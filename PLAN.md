...
- Hard-cutover safety updates now shipped: `--plan` is per-run only, agent timeout is a total run deadline, mutating tools use a dedicated execution lock, and same-session mutation conflicts fail before side effects.
...
- `headless session new` creates the session directory, `meta.json`, empty `messages.jsonl`, a metadata lock file, and an execution lock file immediately. `meta.json` allows `agent_name`, `model`, `effort`, `cwd`, and `initial_role` to be `null` until the first successful prompt run.
- The first successful prompt run permanently binds `agent_name` and establishes session defaults for `model`, `effort`, `cwd`, and `initial_role`. Later runs inherit those values when omitted; `--model`, `--effort`, and `--cwd` may override per run without mutating the stored session defaults. `--plan` is per-run only.
...
- Implement a direct `src/provider/openrouter.rs` module rather than a generic provider trait. Support `model`, `messages`, `tool definitions`, `max_output_tokens`, total-run timeout handling, and `effort`; omit the field when `effort=none`, otherwise pass it through and surface provider rejection as a stable provider error.
- Use lowercase ULIDs for both session ids and run ids. Persist messages in JSONL, create per-run artifact directories, and enforce optimistic concurrency by checking `meta.json.revision` before append. Mutating tools acquire the dedicated execution lock before side effects and same-session mutation conflicts fail with a specific conflict exit code.
...
- Use runtime defaults from the spec, plus step cap `24` and per-tool retry cap `2` for read-only tools only. Default shell execution to `/bin/bash` with `-lc`.
...
- Verify persistence and concurrency: JSONL append shape, artifact preview metadata, catastrophic-output handling, chronological timestamps, conflict failure for concurrent same-session runs, mutating execution-lock behavior, and successful parallel runs across different sessions.
- Verify tool behavior: allowlist enforcement, deterministic `--plan` no-op results for every core tool, valid and invalid multi-file patch behavior, total-run timeout handling, and at least one end-to-end loop test that uses `bash` plus file tools.
...
- Session defaults established on first bind do not change when later runs override `model`, `effort`, or `cwd`.
